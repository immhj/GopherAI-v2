...
func Login(email, password string) (string, code.Code) {
	...
	//3:返回一个Token（内部标识仍为系统生成的 Username）
...
func Register(email, nickname, password, captcha string) (string, code.Code) {
	...
	//3：生成11位的内部账号（用户不可见，作为内部标识）
	username := utils.GetRandomNumbers(11)

	//4：注册到数据库中（昵称由用户填写）
	if userInformation, ok = user.Register(nickname, username, email, password); !ok {
		return "", code.CodeServerBusy
	}
