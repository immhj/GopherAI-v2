...
type (
	...
	ChatSendRequest struct {
		...
		Image        string `json:"image,omitempty"`                        // 可选：图片（多模态）
	}
...
// GetModels 返回可供前端选择的真实模型清单
func GetModels(c *gin.Context) {
	res := new(ModelsResponse)
	cfg := config.GetConfig().ModelServiceConfig
	res.Success()
	res.Models = cfg.Models
	res.DefaultModel = cfg.DefaultModel
...
func CreateSessionAndSendMessage(c *gin.Context) {
	...
	session_id, aiInformation, code_ := session.CreateSessionAndSendMessage(userName, req.UserQuestion, req.Model, req.Image)
...
func ChatSend(c *gin.Context) {
	...
	// 发送消息，并会将AI回答返回
	aiInformation, code_ := session.ChatSend(userName, req.SessionID, req.UserQuestion, req.Model, req.Image)
