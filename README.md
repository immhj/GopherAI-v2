...
- **联网搜索与网页抓取** —— 模型判断问题需要时效信息时自行联网检索，必要时再抓取原页正文
...
| `ARK_API_KEY` | 火山方舟，文档向量化（RAG） | 用文档检索则必填 |
| `TAVILY_API_KEY` | Tavily，联网搜索 | 用 `web_search` 则必填 |
...
| 工具 | 作用 | 依赖 |
|---|---|---|
| `web_search` | 联网搜索（Tavily），返回摘要与带链接的结果 | `TAVILY_API_KEY` |
| `fetch_url` | 抓取网页正文，`web_search` 的搭档 | 无 |
| `search_documents` | 在该用户自己的文档里做语义检索 | `ARK_API_KEY` + Qdrant |
| `calculate` | 计算数学表达式 | 无 |
| `get_weather` | 查城市天气（wttr.in） | 无 |
| `get_current_time` | 查当前日期时间，可指定时区 | 无 |

需要区分的是：**并非所有能力都是工具**。图片理解、会话历史、流式输出是管道内置的，模型没有选择权；上面这 6 个才是交给模型自行判断的。

工具描述每次请求都会随消息发给模型，所以工具数量直接影响 token 成本和选错工具的概率 —— 这里刻意保持克制，没有一股脑堆功能。

### MCP 和「工具」不是一回事

容易混淆，值得写清楚：

- **工具（Tool）** 是能力本身：一个名字、一段描述、一份参数 Schema。
- **MCP** 是一种**协议**，用来把住在**另一个进程**里的工具暴露出来。它是「工具的来源」之一，不是工具本身。

对模型来说两者毫无区别 —— 它只看到工具表里的条目，不知道背后是本地 Go 函数还是经 MCP 转发的远程调用。

因此：自己写的能力**直接内置即可，不需要 MCP**；MCP 的价值在于复用生态里现成的 server。上面 6 个工具全部是内置实现。

⚠️ 一个常见误解：MCP **并不能绕过 API key**。生态里的搜索类 MCP server 自己也要 key（Brave/Tavily 等），走 MCP 只是把 key 的位置换了个地方，还额外多一层协议和一个进程。
...
抓取的网址来自**模型**，而模型可能被提示注入操纵。不加限制的话，它就能把后端当成
访问内网的跳板 —— 去读 `http://mysql:3306`、`http://qdrant:6333`，或云厂商的
`169.254.169.254` 元数据接口。因此有四道限制：

1. **协议白名单** —— 只允许 http / https
2. **地址校验** —— 域名解析后逐个 IP 检查，命中私网 / 回环 / 链路本地 / CGNAT 则拒绝
3. **每跳重校验** —— 重定向的每一跳都重新校验，否则公网域名可以 302 跳进内网
4. **体量限制** —— 响应大小、超时、返回字符数都有上限

有一个网段刻意**没有**拦：`198.18.0.0/15`。它是 RFC 2544 基准测试段，不承载常规内网服务，
而 Clash / sing-box 这类代理在 fake-IP 模式下会把公网域名映射到这个段 —— 拦掉会导致所有
正常网站都访问不了。
...
- **`calculate` 用 float64** —— 整数结果超过 2^53（约 9×10^15）后精度不再保证。日常算术没问题，超大整数运算不要依赖它。
- **网页正文提取是正则实现** —— 剥掉脚本样式标签后取文本，对结构复杂或重度依赖 JavaScript 渲染的页面效果有限。
- **MCP 尚未接入** —— `common/mcp` 下有一个独立的天气 server（自带 go.mod），但没有注册进工具表；同名能力目前由内置的 `get_weather` 提供。
- **语音合成已移除**。
